...
/**
 * Função utilitária para limpar o título removendo emojis e espaços no início.
 * A regex [\p{Emoji}\u200d\uFE0F]u detecta qualquer emoji.
 */
function limparTitulo(titulo) {
    if (!titulo) return "";
    // Remove emojis e símbolos especiais do início do título, incluindo espaços
    return titulo.replace(/^[\p{Emoji}\p{Symbol}\s]+/u, "").trim();
}
...
function extrairFilmes() {
    ...
    for (const [id, list] of Object.entries(movies)) {
        ...
        // Padrão: remove emojis do início e captura o nome e o ano entre parênteses
        const tituloLimpo = limparTitulo(item.title);
        const match = tituloLimpo.match(/^(.+?)\s*\((\d{4})\)/);
...
function extrairSeries() {
    ...
    for (const [id, list] of Object.entries(series)) {
        ...
        const tituloLimpo = limparTitulo(item.title);
        const match = tituloLimpo.match(/^([^(]+?)\s*\((\d{4})\)/i);
